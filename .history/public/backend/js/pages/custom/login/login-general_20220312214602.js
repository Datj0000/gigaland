...
var KTLogin = function () {
    ...
    function _send_token(admin_email){
        $.ajax({
            url: 'admin/send-token',
            method: 'POST',
            headers: {
    ...
    var _handleSignUpForm = function (e) {
        ...
        $('#kt_login_signup_submit').on('click', function (e) {
            e.preventDefault();
            var admin_email = $('#email_forgot').val();
            ...
            validation.validate().then(function (status) {
                if (status == 'Valid') {
                    $.ajax({
                        ...
                        success: function (data) {
                            if (data == 0) {
                                Swal.fire("", "Your token has expired!", "warning");
                            }
                            else if (data == 1) {
            ...
            validation.validate().then(function (status) {
                if (status == 'Valid') {
                    ...
                    $.ajax({
                        ...
                        success: function (data) {
                            if (data == 0) {
                                Swal.fire("", "This email has not been registered for an account!", "warning");
                            }
                            else if (data == 1) {
                                Swal.fire("", "Please check your email to reset your password!", "success");
                                _showForm('signup');
                                _send_token(admin_email);
                            }
                        }
                    })
                } else {
