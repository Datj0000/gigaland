...
class AttendanceController extends Controller
{
    ...
    public function check()
    {
        ...
        //     echo $today;
        // }
        $all_buypackage = BuyPackage::where('status', '0')->get();
        foreach($all_buypackage as $key => $item){
            $item->buypackage_id;
            $all_attendance = Attendance::where('buypackage_id', $item->buypackage_id)->orderBy('attendance_id', $attendance_id)->first();
            echo $all_attendance->attendance_id;
            // if($item2->created_at < $today){
            //     $item->status = 2;
            //     $item->save();
            // }
        }
