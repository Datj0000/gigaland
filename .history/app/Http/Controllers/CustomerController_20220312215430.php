...
class CustomerController extends Controller
{
    ...
    public function update_profile(Request $request)
    {
        ...
        $check_email = Customer::where('customer_email', $data['email'])->where('customer_email', '!=', $admin_id)->first();
        $check_phone = Customer::where('customer_phone', $data['phone'])->where('admin_id', '!=', $admin_id)->first();
        if ($check_username) {
            echo 0;
        } else if ($get_image) {
